...
namespace wrapper
{
    ...
    public sealed class Vmm : IDisposable
    {
        ...
        public unsafe bool ReadUtfUnicode8Memory(ulong address, uint size, out byte[] buffer)
        {
            byte[] array = (buffer = new byte[size + 16]);
            ulong input = 1uL;
            uint result = (uint)input;
            fixed (byte* ptr = array)
            {
                uint pcbReadOpt;
                for (uint i = 0; i < size; i += pcbReadOpt)
                {
                    uint num = size - i;
                    if (num >= 65536)
                    {
                    ...
                    pcbReadOpt = 0;
                    if (!vmmi.VMMDLL_MemReadEx(hVMM, DMA_Rust.mem.memory._pid, address + (ulong)i, ptr + i, num, out pcbReadOpt, result) || pcbReadOpt == 0)
                    {
                        return false;
                    }
